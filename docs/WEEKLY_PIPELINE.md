...
- Workflow validates `artifacts/last_run.json` contract with `scripts/pipeline/validate_last_run_summary.py` before publishing CI summary output, including run-history index parity (`index.json`/`index.md` presence + retained-count consistency).
